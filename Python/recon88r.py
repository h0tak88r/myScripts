...
def check_required_tools():
    ...
    print("[+] Checking if the tools and required files are available")

    # Check if the nuclei_templates directory exists
    nuclei_templates_path = os.path.join(current_directory, "nuclei_templates")
    if not os.path.isdir(nuclei_templates_path):
        print(f"Error: The '{nuclei_templates_path}' directory does not exist. Please ensure it is present before running the script.")
        exit(1)

    # Check if the fuzz wordlist file exists in the current working directory
    fuzz_wordlist = "Wordlists/h0tak88r_fuzz.txt"
    fuzz_wordlist_path = os.path.join(current_directory, fuzz_wordlist)
    if not os.path.isfile(fuzz_wordlist_path):
        print(f"Error: The '{fuzz_wordlist}' wordlist file does not exist in the specified directory. Please provide the correct path or ensure the file is present.")
        exit(1)

    # Check if the required tools are installed
    required_tools = ["notify", "naabu", "httpx", "gau", "nuclei", "cero", "ffuf", "subfinder", "puredns"]
    ...
    for tool in required_tools:
        try:
            ...
            print(f"Error: {tool} returned a non-zero exit code. Please check its installation and configuration.")
            exit(1)

    print("[+] All required tools and files are available.")
